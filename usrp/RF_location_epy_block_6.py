...
class blk(gr.sync_block):  # other base classes are basic_block, decim_block, interp_block
    ...
    def work(self, input_items, output_items):
        ...
        self.now = 2 * self.freqency_list[self.selector]
        # set the center frequency of source
        # 创建一个PMT字典
        command = pmt.make_dict()
        command = pmt.dict_add(command, pmt.intern("freq"), pmt.from_double(self.now_freq))
        command = pmt.dict_add(command, pmt.intern("chan"), pmt.from_long(1))
        command = pmt.dict_add(command, pmt.intern("gain"), pmt.from_double(self.now_gain))
